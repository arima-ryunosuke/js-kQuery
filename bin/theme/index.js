...
import walk from 'documentation/src/walk.js';
...
export default async function (comments, config) {
  walk(comments, function (comment) {
    const descriptor = comment.tags.find(tag => tag.title === 'descriptor');
    if (descriptor) {
      comment.name = descriptor.description + ' ' + comment.name;
      const parts = comment.namespace.split('#');
      if (parts[1]) {
        parts[1] = descriptor.description + '__' + parts[1];
      }
      comment.namespace = parts.join('#');
    }
  }, config);
  ...
  const path = new class {
    join(...paths) {
      ...
      paths[0] = fileURLToPath(import.meta.resolve('documentation') + '/../default_theme');
      return paths.join('/');
    }
  }();
